...
const MapView = () => {
  ...
  const [activeTab, setActiveTab] = useState<'Tiger' | 'Elephant'>('Tiger');
  const { data: tigerMarkers = [], isLoading: isLoadingTigers } = useAnimalLocations('Tiger');
  const { data: elephantMarkers = [], isLoading: isLoadingElephants } = useAnimalLocations('Elephant');
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500">
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight mb-2">Map View</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-4 items-start">
        <div className="space-y-4 lg:col-span-1">
          <Card className="glass-card p-4">
            <div className="flex gap-2 mb-3">
              <Button variant={activeTab === 'Tiger' ? 'default' : 'outline'} size="sm" onClick={() => setActiveTab('Tiger')}>Tigers</Button>
              <Button variant={activeTab === 'Elephant' ? 'default' : 'outline'} size="sm" onClick={() => setActiveTab('Elephant')}>Elephants</Button>
            </div>
            ...
            {activeTab === 'Tiger' ? (
              isLoadingTigers ? (
                ...
                <div className="space-y-3">
                  {tigerMarkers.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-4">No recent tiger sightings</p>
                  ) : (
                    tigerMarkers.map((marker) => (
                      <div
                        key={marker.id}
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <Badge variant="outline" className="gradient-tiger border-0 text-xs">🐅</Badge>
                        </div>
              ...
              isLoadingElephants ? (
                ...
                <div className="space-y-3">
                  {elephantMarkers.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-4">No recent elephant sightings</p>
                  ) : (
                    elephantMarkers.map((marker) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <Badge variant="outline" className="gradient-elephant border-0 text-xs">🐘</Badge>
                        </div>
                        <p className="text-sm font-medium mb-1">{marker.name}</p>
                        <p className="text-xs text-muted-foreground">{marker.location}</p>
                        <p className="text-xs text-muted-foreground mt-1">
        ...
        <div className="lg:col-span-3">
          <Card className="glass-card p-1 relative overflow-hidden border-2 border-border shadow-lg" style={{ height: 'auto', minHeight: '800px' }}>
            {!hasApiKey ? (
